...
/**
 * Converts source building height metadata to metres for rendering.
 * The transformation method must be recorded separately when the value is
 * promoted into engineering evidence; this function itself is not authority.
 */
export function normalizeBuildingHeight(props: Record<string, unknown> | null): number {
  ...
  const explicit = [
    ...
    props.eaveheight,
    props.EaveHeight,
  ].map(numericValue).find((v): v is number => v !== undefined);
...
function enrich(features: GeoJSONFeature[]): GeoJSONFeatureCollection {
  return {
    ...
    features: features.map((feature) => ({
      ...
      properties: {
        ...(feature.properties ?? {}),
        height_m: normalizeBuildingHeight(feature.properties),
      },
    })),
  };
}

/** Deterministic local forensic structures used when authoritative remote data is unavailable. */
export function getLocalBonebankBuildings(): GeoJSONFeatureCollection {
  const [lon, lat] = BONEBANK_CENTER;
  const d = 0.00035;
  return enrich([
    {
      type: "Feature",
      properties: { id: "bonebank-primary", name: "13101 Bonebank Rd Primary Structure", height_m: 7.2, levels: 1, source: "local-forensic" },
      geometry: { type: "Polygon", coordinates: [[[lon - d * 1.2, lat - d * 0.8], [lon + d * 1.4, lat - d * 0.8], [lon + d * 1.4, lat + d * 0.9], [lon - d * 1.2, lat + d * 0.9], [lon - d * 1.2, lat - d * 0.8]]] },
    },
    {
      type: "Feature",
      properties: { id: "bonebank-outbuilding", name: "Accessory Structure", height_m: 4.5, levels: 1, source: "local-forensic" },
      geometry: { type: "Polygon", coordinates: [[[lon + d * 1.8, lat - d * 0.4], [lon + d * 2.6, lat - d * 0.4], [lon + d * 2.6, lat + d * 0.3], [lon + d * 1.8, lat + d * 0.3], [lon + d * 1.8, lat - d * 0.4]]] },
    },
  ]);
}
...
async function fetchGeoJSON(url: string): Promise<GeoJSONFeatureCollection> {
  try {
    ...
    return data?.features ? data : EMPTY_FC;
  } catch {
    return EMPTY_FC;
  }
}
...
async function fetchMicrosoftClip(bbox: BBox): Promise<GeoJSONFeatureCollection> {
  const url = import.meta.env.VITE_MS_BUILDINGS_CLIP_URL ?? "/data/buildings/indiana_bonebank_clip.geojson";
...
/**
 * Authoritative source priority is local forensic evidence, Microsoft clip,
 * then Overture. Remote sources remain provenance-bearing inputs; the local
 * sample is deterministic fallback, not a claim of survey authority.
 */
...
export function fetchBonebankBuildings(): Promise<GeoJSONFeatureCollection> {
  return fetchBuildings(BONEBANK_BBOX);
}
