...
export function addLayer19Buildings(
  map: maplibregl.Map,
  ...
  return {
    ...
    setOpacity(opacity) {
      ...
      map.setPaintProperty(LAYER19_EXTRUSION, "fill-extrusion-opacity", clamped);
    },
    setHeightScale(scale) {
      const clamped = Math.max(0, scale);
      map.setPaintProperty(LAYER19_EXTRUSION, "fill-extrusion-height", ["*", ["coalesce", ["get", "height_m"], 6.5], clamped]);
    },
    remove() {
      if (map.getLayer(LAYER19_OUTLINE)) map.removeLayer(LAYER19_OUTLINE);
      if (map.getLayer(LAYER19_EXTRUSION)) map.removeLayer(LAYER19_EXTRUSION);
